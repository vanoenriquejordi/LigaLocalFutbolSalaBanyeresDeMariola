...
import tfc.jordivanoenrique.ligalocalfutbolsalabanyeresdemariola.databinding.ItemTeamBinding
import tfc.jordivanoenrique.ligalocalfutbolsalabanyeresdemariola.entities.Clasificacion
import tfc.jordivanoenrique.ligalocalfutbolsalabanyeresdemariola.entities.Equipo
import tfc.jordivanoenrique.ligalocalfutbolsalabanyeresdemariola.utils.FragmentAux

class TeamFragment : Fragment(), FragmentAux {

    private lateinit var mBinding: FragmentHomeBinding

    private lateinit var mFirebaseAdapter: FirebaseRecyclerAdapter<Equipo, SnapshotHolder>
    ...
    private fun setupFirebase() {
        mSnapshotsRef = FirebaseDatabase.getInstance().reference.child(LLFSApplication.PATH_TEAM)
    }

    private fun setupAdapter() {
        val query = mSnapshotsRef

        val options = FirebaseRecyclerOptions.Builder<Equipo>().setQuery(query) {
            val snapshot = it.getValue(Equipo::class.java)
            snapshot!!.id = it.key!!
            snapshot
        }.build()

        mFirebaseAdapter = object : FirebaseRecyclerAdapter<Equipo, SnapshotHolder>(options) {
            ...
            override fun onBindViewHolder(holder: SnapshotHolder, position: Int, model: Equipo) {
                val snapshot = getItem(position)

                with(holder) {

                    with(binding) {
                        tvNumJugadores.text = snapshot.numJugadores
